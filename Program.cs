...
if (!Enum.TryParse(configuration["logLevel"], out minLogLevel)) {
    minLogLevel = LogLevel.Debug;
}

using ILoggerFactory factory = LoggerFactory.Create(builder => builder.AddConsole().SetMinimumLevel(minLogLevel));
ILogger logger = factory.CreateLogger<Program>();

logger.LogInformation("Starting qbit-stats");
